...
export default class Game {
  constructor(columns, rows) {
    ...
    this.columns = columns || this.levelData.columns;
    this.rows = rows || this.levelData.columns;
  ...
  callbackToChangeClickState(e, game){
    console.log(game.grid.cellArray);
    let el = e.target;
  ...
  setupEvents(callback){
    ...
    $('#winGameModal').on('click', function(e){
      self.playAgain(e, self);
    });
    $('#loseGameModal').on('click', function(e){
      self.playAgain(e, self);
    });
  ...
  playAgain(e,self){
    //debugger;
    if(e.target && e.target.id === "nextLevel"){
      //next level button clicked on win modal
      self.currentLevel++;
    }
    if(e.target && e.target.id === "startover"){
      //start  over button clicked on lose modal
      self.currentLevel = 1;
    }
    if(e.target && e.target.id === "restart"){
      //restart button clicked on lose modal
      //self.currentLevel = self.currentLevel;
    }

    self.lives = self.resetLives();
    self.restart();
    /*
